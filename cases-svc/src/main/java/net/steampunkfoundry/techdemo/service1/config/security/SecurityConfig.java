package net.steampunkfoundry.techdemo.service1.config.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        http.requestMatchers().and().authorizeRequests()
                .antMatchers("/configuration/ui", "/swagger-ui.html", "/actuator/health", "/swagger-resources/**",
    ...
    // based on
    // https://kevcodez.de/posts/2020-03-26-secure-spring-boot-app-with-oauth2-aws-cognito/#check-user-groups
    private Collection<GrantedAuthority> convertGroupsToAuthorities(Jwt jwt) {
        return (Set) ((List) jwt.getClaims().getOrDefault("cognito:groups", new ArrayList<>())).stream()
                .map(t -> new SimpleGrantedAuthority((String) t)).collect(Collectors.toSet());
    }
