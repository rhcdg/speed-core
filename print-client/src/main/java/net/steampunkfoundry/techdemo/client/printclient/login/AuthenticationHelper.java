...
/**
 * This class was provided by Amazon for integration with the Cognito API.
 */
public class AuthenticationHelper {
    ...
    public AuthenticationHelper(String userPoolID, String clientid, String region) {
        do {
            ...
            bigA = g.modPow(a, N);
        } while (bigA.mod(N).equals(BigInteger.ZERO));
    ...
    private byte[] getPasswordAuthenticationKey(String userId,
        ...
        MessageDigest messageDigest = THREAD_MESSAGE_DIGEST.get();
        messageDigest.reset();
        messageDigest.update(bigA.toByteArray());
        ...
        try {
            ...
            if (ChallengeNameType.PASSWORD_VERIFIER.toString()
                ...
                RespondToAuthChallengeRequest challengeRequest = userSrpAuthRequest(
                        initiateAuthResult, password,
                        initiateAuthRequest.getAuthParameters().get("SECRET_HASH"));
                RespondToAuthChallengeResult result = cognitoIdentityProvider
                ...
                JSONObject cognitoPayload = CognitoJwtParser
                        .getPayload(result.getAuthenticationResult().getIdToken());
                if (isAdjudicator(cognitoPayload)) {
                    jwtToken = result.getAuthenticationResult().getIdToken();
                    SecurityContextHolder.getContext()
                            .setAuthentication(new JwtAuthenticationToken(
                                    Jwt.withTokenValue(jwtToken).header("name", "value")
                                            .claim("name", "value").build()));
                    Logger.getLogger(AuthenticationHelper.class.getName())
                            .log(Level.INFO, "jwt: " + jwtToken);
                }
            }
    ...
    private boolean isAdjudicator(JSONObject cognitoPayload) {
        JSONArray cognitoGroups = (JSONArray) cognitoPayload.get("cognito:groups");
        for (Object o : cognitoGroups) {
            if ("Adjudicator".equals(o)) {
                return true;
            }
        }
        return false;
    ...
    private InitiateAuthRequest initiateUserSrpAuthRequest(String username) {
        ...
        initiateAuthRequest.addAuthParametersEntry("SRP_A", this.getBigA().toString(16));
    ...
    private RespondToAuthChallengeRequest userSrpAuthRequest(InitiateAuthResult challenge,
        ...
        try {
            ...
            byte[] secretBlock = Base64.getDecoder()
        ...
        SimpleDateFormat formatTimestamp = new SimpleDateFormat("EEE MMM d HH:mm:ss z yyyy",
                Locale.US);
        ...
        srpAuthResponses.put("PASSWORD_CLAIM_SIGNATURE",
                new String(Base64.getEncoder().encode(hmac), StringUtils.UTF8));
        srpAuthResponses.put("TIMESTAMP", formatTimestamp.format(timestamp));
        srpAuthResponses.put("USERNAME", challenge.getChallengeParameters().get("USERNAME"));
    ...
    static final class Hkdf {
        ...
        private static final int maxKeySize = 255;
        ...
        private void init(byte[] ikm, byte[] salt) {
            byte[] realSalt = salt == null ? emptyArray : (byte[]) salt.clone();
        ...
        private void deriveKey(byte[] info, int length, byte[] output, int offset)
            ...
            if (length < 0) {
                ...
                if (length > maxKeySize * mac.getMacLength()) {
